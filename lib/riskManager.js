/**
 * lib/riskManager.js — Risk Management v4 + Ultra Profit Mode
 */
...
// ── Ultra Profit Sizing (Lebih Agresif) ───────────────────────────
export function calculateUltraPositionSize(totalBalance, openPositionsCount, botState = {}, signalGrade = 'C') {
  ...
  if (consecutiveWins >= 5)      multiplier = 1.45;
  else if (consecutiveWins >= 3) multiplier = 1.28;
  else if (consecutiveWins >= 2) multiplier = 1.12;

  if (totalPnl > 800000)         multiplier = Math.max(multiplier, 1.35);
  else if (totalPnl > 400000)    multiplier = Math.max(multiplier, 1.20);

  base.idrAmount = Math.floor(base.idrAmount * multiplier);
  base.reason = `ultra_${consecutiveWins}win_streak`;
